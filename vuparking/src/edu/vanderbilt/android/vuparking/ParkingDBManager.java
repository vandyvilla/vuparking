...
public class ParkingDBManager
{
	...
	public ArrayList<ParkingLot> queryParkingZone(int zone){
		...
		Cursor c = adapter.getLotByZone(zone);
		if (c.moveToFirst()){
			do {
				ParkingLot p = new ParkingLot();
				p.setName(c.getString(c.getColumnIndex(ParkingDBAdapter.KEY_NAME)));
				p.setAddress(c.getString(c.getColumnIndex(ParkingDBAdapter.KEY_ADDRESS)));
				p.setNumSpot(c.getInt(c.getColumnIndex(ParkingDBAdapter.KEY_CAPACITY)));
				p.setNumAvailable(c.getInt(c.getColumnIndex(ParkingDBAdapter.KEY_AVAILABLE)));
				p.setNumDisable(c.getInt(c.getColumnIndex(ParkingDBAdapter.KEY_DISABLE)));
				p.setCoordinates(c.getDouble(c.getColumnIndex(ParkingDBAdapter.KEY_COORDINATE_X)),
